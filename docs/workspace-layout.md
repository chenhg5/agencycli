...
What an agent does when it wakes up with an empty task queue. Referenced from `project.yaml` via `playbook:` and installed as `wakeup.md` by `project apply`.
...
### `projects/<project>/agents/<agent>/tasks.yaml`
...
When you run `hire` (or `project apply`), agencycli merges the context chain:
...
The result is written as a single context file (`CLAUDE.md`, `AGENTS.md`, etc.) in the agent's working directory. Change any layer and run `agencycli sync` to propagate — only agents affected by the changed file are updated.
