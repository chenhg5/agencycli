...
func (m *systemdManager) buildUnit(cfg Config) string {
	...
	fmt.Fprintf(&sb, "Environment=AGENCYCLI_LOG_FILE=%s\n", cfg.LogFile)
	fmt.Fprintf(&sb, "Environment=AGENCYCLI_LOG_MAX_SIZE=%d\n", cfg.LogMaxSize)
	if cfg.EnvPATH != "" {
...
func checkSystemdRunning(system bool) error {
	...
	if system {
		if wsl {
			return fmt.Errorf("systemd is not active in this WSL2 instance.\n" +
				...
				"    [boot]\n" +
				"    systemd=true\n" +
				"  Or use: nohup agencycli start > agencycli.log 2>&1 &")
		}
		if state == "offline" || strings.Contains(state, "not been booted") {
			return fmt.Errorf("systemd is not active (state: %s).\n"+
				...
				"    nohup agencycli start > agencycli.log 2>&1 &", state)
		}
		return fmt.Errorf("systemd check failed (state: %s).\n"+
			"  Use nohup as alternative: nohup agencycli start > agencycli.log 2>&1 &", state)
	}
	...
	return fmt.Errorf("systemd user session not available.\n"+
		...
		"  Try one of:\n"+
		"    1. Run as root: sudo agencycli daemon install (uses system-level systemd)\n"+
		"    2. loginctl enable-linger %s && export XDG_RUNTIME_DIR=/run/user/$(id -u)\n"+
		"    3. Use nohup/tmux instead: nohup agencycli start > agencycli.log 2>&1 &", user)
}
