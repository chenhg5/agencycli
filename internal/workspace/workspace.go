...
//
// An agencycli workspace is any directory that contains a .aios/agency.yaml
...
// .aios/agency.yaml, then returns that directory's absolute path.
		...
		if parent == dir {
			return "", fmt.Errorf(
				"not inside an agencycli workspace "+
					"(no .aios/agency.yaml found in %q or any parent directory)",
				abs,
			)
		}
		dir = parent
