...
const (
	...
	AIOSDir = ".aios"
	// CompanyFile is the company config file inside AIOSDir.
	CompanyFile = "company.yaml"
)

// FindRoot walks up from start until it finds a directory that contains
// .aios/company.yaml, then returns that directory's absolute path.
func FindRoot(start string) (string, error) {
	...
	dir := abs
	for {
		marker := filepath.Join(dir, AIOSDir, CompanyFile)
